...
> **状态**: ✅ 已完成（2026-08-04）—— 站点活着，6 条验收全过，README 演示链接已加
> **依赖**: 04 ✅
> **阶段**: 本地化收尾
...
### Step 3 — 上线后回写 README ✅（2026-07-27 完成，`c400a5e`）
...
> **完成日期**: 2026-08-04（上线 2026-07-27，08-04 复测后闭合）
> **执行人**: 作者（Vercel 项目 + DNS）· Claude Code（验收、修复、文档）

### 执行摘要

| 检查项 | 结果 |
|--------|:--:|
| Step 0 push | ✅ `master == origin/master`（0 ahead / 0 behind） |
| Step 1 Vercel 项目 + `READ_ONLY=1` | ✅ 部署成功 |
| Step 2 域名解析 | ✅ <https://cook.wreathmoon.com> 可访问 |
| Step 3 README 演示链接 | ✅ [README.md:9](../README.md)（`c400a5e`） |
| 6 条验收 | ✅ 全过（明细见上「验收实测」表） |

### 2026-08-04 线上复测（浏览器实跑，非静态抓取）

| 项 | 实测值 |
|----|--------|
| 只读横幅 | ✅「只读演示实例 —— 改动不会被保存」 |
| 库存 / 厨具 | ✅ 49 种在库食材 · 4 件厨具 |
| 推荐分层 | ✅ 今日推荐 2 道（均标「该清库存了」）+ 备选推荐 10 道 |
| 购物清单 | ✅ 1 项（豆干 · 库存不足） |

> ⚠️ **复测方法上的一个坑，记下来免得下次误判**：用纯 HTTP 抓取（不执行 JS）看这个站，
> 会得到「0 食材 / 0 厨具 / 0 道能安排」——因为页面是 Client Component，首屏数据在
> `useEffect` 里拉（[SPEC.md](../SPEC.md) §3.3）。**那不是回归，是抓取方式看不见客户端渲染。**
> 验收这个站必须用真浏览器。

### 本任务未产出新代码

沙盒的全部代码在 [Task/04](./04-single-user-local-✅已完成.md) 就已就位（`assertWritable()` / `ReadOnlyProvider` / `outputFileTracingIncludes`）。
本任务只做部署与验收——但**验收过程本身挖出了两个全局缺陷**，都不在只读逻辑里：

| 缺陷 | 归属 |
|------|------|
| antd × React 19 导致全项目 68 处 `message.*` 静默失效 | `163d02d`，记入 [Task/16](./16-global-ui-defects-✅已完成.md) |
| 全站 6 条路由都滚不动 | `b33581f`，记入 [Task/16](./16-global-ui-defects-✅已完成.md) |

### 已定决策（无新增）

四条关键决策在开工前即已定案，执行中无一被推翻：读 `seed/` 不读 `data/`、
写入拒绝走服务端 `assertWritable()`、不做访客数据隔离、`READ_ONLY` 是必填而非可选。

### 教训

**「本地跑得通」和「线上用户看得见」之间隔着两个全局缺陷。** 本任务代码侧 100% 就绪、
6 张证据表齐全，真部署上去仍立刻撞出 toast 全哑与全站滚不动——两个都是渲染层的、
本地开发路径恰好绕开的问题。**部署不是收尾动作，它是第一次真实验收。**
