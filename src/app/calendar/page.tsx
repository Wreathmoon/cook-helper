...
export default function CalendarPage() {
  ...
  // 带取消标记的 async IIFE：setState 落在 await 之后，切月份/卸载时不再写旧结果
  // （也顺带满足 react-hooks/set-state-in-effect）
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await getCalendarEntriesAction(month.year(), month.month() + 1);
        if (cancelled) return;
