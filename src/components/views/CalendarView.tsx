...
export interface CalendarViewProps {
  ...
  loading?: boolean;
  onAddEntry: (params: { date: string; recipeId: string; status: CalendarStatus }) => Promise<void>;
  onDeleteEntry?: (entryId: string) => Promise<void>;
  onCompleteEntry?: (entryId: string) => Promise<void>;
  /** 打开"我做完了"弹窗时获取食材信息 */
  onFetchDoneIngredients?: (entry: EntryWithRecipe) => Promise<DoneIngredient[]>;
  /** "我做完了"提交：更新库存+标记完成 */
...
const labelStyle: CSSProperties = { fontSize: 12, color: 'var(--tx2)', marginBottom: 4 };
...
export function CalendarView({
  ...
  const [doneIngredients, setDoneIngredients] = useState<DoneIngredient[]>([]);
  ...
  const handleAdd = async () => {
    ...
    try {
      await onAddEntry({ date: addDate.format('YYYY-MM-DD'), recipeId: addRecipeId, status: addStatus });
      message.success('已添加');
      setAddOpen(false);
  ...
  const handleQuickAdd = async (recipeId: string) => {
    ...
    try {
      await onAddEntry({ date: selectedDate.format('YYYY-MM-DD'), recipeId, status: 'planned' });
      message.success('已添加');
  ...
  const handleComplete = async (entryId: string) => {
    if (!onCompleteEntry) return;
    try {
      await onCompleteEntry(entryId);
    } catch { message.error('操作失败'); }
  };
  ...
  const openDoneModal = async (entry: EntryWithRecipe) => {
    if (!onFetchDoneIngredients) return;
    try {
      const ings = await onFetchDoneIngredients(entry);
  ...
  const handleDoneSubmit = async () => {
    if (!onDoneSubmit || !doneEntry) return;
    setDoneLoading(true);
    try {
      const updates = Object.entries(doneNewLevels).map(([id, stock_level]) => ({ id, stock_level }));
      await onDoneSubmit(doneEntry.id, updates);
      message.success('库存已更新');
      setDoneModal(false);
    } catch { message.error('操作失败'); } finally { setDoneLoading(false); }
  };

  // 月视图：固定 5 行 35 格，周一为一周首日
    ...
    <div className="page-body">
      ...
      >
        <div style={{ marginBottom: 12 }}>
          ...
          <input type="date" value={addDate.format('YYYY-MM-DD')}
        ...
        <div>
          ...
          <input type="text" placeholder="搜索菜谱..." value={addRecipeSearch}
            onChange={(e) => setAddRecipeSearch(e.target.value)} style={inputStyle} />
